...
This project provides standalone nginx binaries for *any* Linux system footnote:[nginx binaries are built as standalone static executables, so they works on every Linux system regardless of used libc.] (x86_64, aarch64), Windows (x86_64).

ifndef::npm-readme[]

You can also download the binaries manually from https://common-binaries.github.io/nginx/ or https://github.com/{gh-name}/tree/binaries[binaries] branch (see <<Files Repository>>).
...
The built binaries are stored in https://github.com/{gh-name}/tree/binaries[binaries] branch of this git repository which is published on https://common-binaries.github.io/nginx/.
...
* {nginx-docs-uri}/http/ngx_http_sub_module.html[ngx_http_sub_module]
